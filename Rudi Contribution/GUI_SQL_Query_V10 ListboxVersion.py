...
def int_selection_1(evt):
    global intersection_1, table_name, int_2, int_select_1, int_select_2, reset_fcn, month_select, intersection_2
    if reset_fcn is False:
        try:
            intersection_1 = int_1_list.get(int_1_list.curselection())
        except:
            pass
        print(intersection_1)

        intersection_1 = intersection_1.replace("'", "''")
        if int_select_2 is False and int_select_1 is False:
            ...
            #Deletes current list of 2nd intersections and replaces it with relevant options.
                ...
                int_2_list.select_set(0)
                
                intersection_2 = (int_2_list.get(ACTIVE))
                intersection_2 = intersection_2.replace("'", "''")
...
def int_selection_2(evt):
    global intersection_2, table_name, int_1, int_select_1, int_select_2, reset_fcn, months, intersection_1
    if reset_fcn is False:
        try:    
            intersection_2 = int_2_list.get(int_2_list.curselection())
        except:
            pass
        intersection_2 = intersection_2.replace("'", "''")
        ...
        if int_select_1 is False and int_select_2 is False:
            ...
            #Deletes current list of 2nd intersections and replaces it with relevant options.
                ...
                int_1_list.select_set(0)
                intersection_1 = (int_1_list.get(ACTIVE))
                intersection_1 = intersection_1.replace("'", "''")
...
def month_selection(x, y, z):
    global months, dates, reset_fcn, int_select_1, int_select_2
    if reset_fcn is False:
        month = month_select.get()

        #Creates a query that only selects the first intersections where the selected intersection exists.
        SQL = "SELECT distinct(date) FROM " + table_name + " WHERE month ILIKE '" + month + "' AND int1 ILIKE '" + intersection_1 + "' AND int2 ILIKE '" + intersection_2 + "'"
        dataframe = pd.read_sql_query(SQL, conn)
        if len(dataframe)<1:
            alert(title='Warning', text="Data doesn't exist for your selection", button="Try again")
        else:
...
def FinishQuery():
    ...
    intersection_query = " WHERE Int1 ILIKE '" + intersection_1 + "' AND Int2 ILIKE '" + intersection_2 + "' "
...
row = Frame(SQL_Query)
options_label3 = Label(row, width=80, text="Table: private_veh_traveltime", anchor='w', font=("Helvetica", 10))
row.pack(side=TOP, fill=X)
...
if len(int_1)>0:
    ...
    int_1_list.bind('<<ListboxSelect>>', int_selection_1)
    ...
    int_1_list.pack(side= RIGHT, fill=X, expand=YES, pady=10)
    ...
    int_2_list.bind('<<ListboxSelect>>', int_selection_2)
    
    row.pack(side=TOP, fill=X)
    lab3.pack(side=LEFT, padx=5)
    filter_2.pack(side=LEFT, padx=10)

    scrollbar2.pack(side=RIGHT, fill=Y)
    int_2_list.pack(side= RIGHT, fill=X, expand=YES, pady=10)
    ...
    row.pack(side=TOP, fill=X)
    lab4.pack(side=LEFT)
    month_list.pack(side= LEFT)
    date_list.pack(side=LEFT)
